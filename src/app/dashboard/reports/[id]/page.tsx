...
export default async function ReportDetailPage({ params }: ReportDetailPageProps) {
  ...
  const canUpdate =
    profile.role === "admin" ||
    (profile.role === "officer" && report.assigned_to === profile.id);
  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[1fr_360px]">
      <article className="grid gap-5">
        ...
        <ReportMap
          reports={[report]}
          detailBasePath="/dashboard/reports"
      ...
      <aside className="grid gap-5 self-start">
        <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <h2 className="text-lg font-bold text-slate-950">Lokasi</h2>
          <div className="mt-4 grid grid-cols-2 gap-3">
            <div className="rounded-lg bg-slate-50 p-3">
              <p className="text-xs text-slate-500">Latitude</p>
              <p className="mt-1 text-sm font-semibold">{report.latitude.toFixed(6)}</p>
            </div>
            <div className="rounded-lg bg-slate-50 p-3">
        ...
        {canUpdate ? (
          <div className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
            <h2 className="mb-4 text-lg font-bold text-slate-950">Update Progres</h2>
